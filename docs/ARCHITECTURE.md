...
- **اسأل المنصّة** (`Ai\Ask`): قراءةٌ فقط بستِّ أدوات (الخمسُ على سجلِّ الوحدات، و`hub_findings` على
  ملاحظات المدقّق عبر `AuditorSignals` نفسِه)، وذاكرةُ محادثةٍ لصاحبها وحدَه (`AskMemory`: مشفَّرة،
  وجوابُها يُعاد تحقّقُه عند كلِّ عرض، والمتابعةُ بالأسئلة لا بالأجوبة)، كلٌّ منها يمرّ بـ`hub_scope` + `hub_can` + `hub_field_mode`؛
  `WRITE_TOOLS = []`.
