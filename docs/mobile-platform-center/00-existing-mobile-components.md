...
| الجلسات | `mobile_sessions` (`App\Models\MobileSession`) + `App\Support\Mobile\MobileSessionService` (revokeSession/revokeAllForUser) |
...
| الدفع (مزوّد/حالة) | `App\Support\Mobile\PushService::status()` + `Push\{Fcm,Null}PushProvider` |
...
| مخطّطُ الرابط العميق | `App\Support\Collaboration\NotificationLink` (`{module,id,action}`) |
| Mobile API / المسارات / OpenAPI | `App\Support\Mobile\MobileOpenApi::{spec,capabilities,mobileRoutes}()` + `GET /api/mobile/v1/openapi.json` |
...
| الأحداثُ الأمنيّة | `App\Support\Security\SecurityEvents` (+ `SecurityRadar`) — لا مخزنَ أحداثٍ جديد |
| التدقيق | `hub_audit()` / `App\Models\AuditEntry` (source=mobile) — لا سجلَّ تدقيقٍ موازٍ |
| الصحّة | نمطُ `App\Support\Ops\Health` (status/label/why/tone + worst-wins) |
| الوثائق | `docs/mobile-readiness/*` + `MobileDocsController` |

## ما يُنشئه المركزُ (طبقةُ عرضٍ فقط — لا خلفيّةَ جديدة)

- `App\Support\Mobile\MobilePlatform` — **خدمةُ قراءةٍ مُجمِّعة** تستدعي المصادرَ أعلاه (لا تخزّن ولا تكرّر).
