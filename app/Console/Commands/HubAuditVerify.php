...
class HubAuditVerify extends Command
{
    ...
    public function handle(): int
    {
        ...
        /*
         * **ملاحظاتُ «نجاحٍ بملاحظات» تُجمع لتُقال، لا لتُعَدّ.**
         *
         * كان الحكمُ `warn` يُخزَّن ويُعرض («⚠️ سليمة بملاحظات · ١٣٢٧ قيداً») بلا
         * سبيلٍ لمعرفة **ما** الملاحظات: الوسمُ في الشاشتين، والتفصيلُ في سطورٍ
         * متفرّقةٍ تتبخّر مع الطرفية. وحكمٌ مشروطٌ بلا تفسير يفتح سؤالَ المدقّق
         * («ما هذه الملاحظات؟») ولا يُغلقه — أسوأُ من لا حكم. فتُجمع هنا مرّةً
         * واحدةً ثم تُقال في الثلاثة معاً: مخرَجُ الأمر (وهو نصُّ فلاشِ زرِّ مركز
         * التشغيل حرفياً)، و`message` في `audit_verifications` الذي تعرضه شاشةُ
         * التدقيق ومركزُ التشغيل، وصفُّ الانتباه.
         *
         * ولكلِّ ملاحظةٍ صيغتان: مطوّلةٌ للطرفية، وموجزةٌ للصفّ المخزَّن (عمودُه
         * ٥٠٠ حرفاً يقصّه `hub_fit` — وملاحظةٌ مبتورةٌ في منتصفها لا تُقرأ).
         */
        $notes = [];   // [['s' => موجزة, 'l' => مطوّلة], …]

        if ($mismatch) {
            $msg = "{$mismatch} قيداً يخالف عمودُ الشركة فيه شركةَ سجلِّه الحالية"
            ...
            $notes[] = ['s' => "{$mismatch} قيداً يخالف عمودُ الشركة شركةَ سجلِّه (نقلٌ مشروع أو تبديل — راجعها)", 'l' => $msg];
            if ($this->option('strict')) { $this->error('⚠️ ' . $msg); $strictFail = true; }
        }
        ...
        if ($blank) {
            $msg = "{$blank} قيداً بلا شركة وسجلُّه اليوم داخل شركة"
            ...
            $notes[] = ['s' => "{$blank} قيداً بلا شركة وسجلُّه داخل شركة (يختفي عن كلِّ نطاق — راجعها)", 'l' => $msg];
            if ($this->option('strict')) { $this->error('⚠️ ' . $msg); $strictFail = true; }
        }
        ...
        if ($weak) {
            // الترتيبُ مقصود: بصمةُ الجيل الأول أوّلُ الملاحظات لأن لها علاجاً معروفاً
            $fix = $this->option('reseal') ? ' (تُرقّى الآن بـ--reseal)' : ' رقّها بـ: php artisan hub:audit-verify --reseal';
            array_unshift($notes, [
                's' => "{$weak} سجلاً ببصمة الجيل الأول (لا تحمي الجهازَ وعنوانَ IP) —" . $fix,
                'l' => "{$weak} سجلاً مختوماً ببصمة الجيل الأول — لا تحمي عمودَي الجهاز وعنوان IP." . $fix,
            ]);
        }
        ...
        // **الملاحظاتُ تُطبع مرقّمةً حيث تُعرض النتيجة** — مخرَجُ الأمر هو نصُّ
        // الفلاش الذي يقرؤه من ضغط زرَّ الفحص في مركز التشغيل، بلا طرفية.
        if ($notes) {
            $this->warn('⚠️ السلسلة سليمة، ومعها ' . (count($notes) === 1
                ? 'ملاحظةٌ واحدة تستحق عيناً:' : count($notes) . ' ملاحظاتٍ تستحق عيناً:'));
            foreach ($notes as $i => $note) $this->warn('   ' . ($i + 1) . ') ' . $note['l']);
        }

        // warn: نجاحٌ بملاحظاتٍ تستحق عيناً (بصمة جيلٍ أول / اختلافُ عمود الشركة) — لا «سليم» يبتلعها.
        // والرسالةُ تحمل **نصَّ** الملاحظات لا عددَها: هي كلُّ ما تعرضه الشاشتان بعد انقضاء الفلاش.
        $this->record($notes ? 'warn' : 'ok', $counters, null,
            "السلسلة سليمة: {$ok} سجل متحقق"
            . ($legacyRows ? " · {$legacyRows} سجل سابق للسلسلة (خارج التحقق)" : '')
            . ($notes ? ' · ملاحظات (' . count($notes) . '): '
                . implode(' | ', array_map(fn ($nt) => $nt['s'], $notes)) : ''));
