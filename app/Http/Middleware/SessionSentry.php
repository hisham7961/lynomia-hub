...
class SessionSentry
{
    ...
    public function handle(Request $r, Closure $next)
    {
        ...
        if (auth()->check() && ! $r->routeIs(...self::SKIP)) {
            $u = auth()->user();
            // `isActive` هو الحكمُ نفسُه الذي يحكم بوابةَ الدخول (F31): null يُقرأ
            // نشطاً كما كان، والمطبَّعُ يُقارَن على الثابت — فلا يفترق الباب والحارس
            if ($u->deleted_at !== null || ! $u->isActive()) {
            ...
            // **لا دخول إلا بالبريد وكلمة المرور** (v2.354): جلسةٌ أُعيد بعثها من كعكة
            // «تذكّرني» (viaRemember) دخولٌ بلا كلمة مرور — كعكةٌ مسروقةٌ أو عودةٌ بعد
            // انتهاء الجلسة تكفي وحدها. النظامُ لم يعد يُصدر الكعكة (remember: false)،
            // وأيُّ كعكةٍ قديمة باقيةٍ في متصفّحٍ تُردّ هنا إلى بوابة الدخول: لا عودةَ
            // إلا بالبريد وكلمة المرور. `kick` يستدعي `Auth::logout` فيُدوّر رمزَ
            // «تذكّرني» فتبطل الكعكةُ القديمة نهائياً. الشرطان معاً يصيبان الجلسةَ
            // المُعادة وحدها — actingAs والجلسات العادية viaRemember=false فلا تُمسّ.
            if (Auth::viaRemember() && ! session('hub.sl')) {
                return $this->kick($r, 'انتهت الجلسة — سجّل الدخول بالبريد وكلمة المرور');
            }
        }
