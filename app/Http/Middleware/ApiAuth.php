...
class ApiAuth
{
    public function handle(Request $request, Closure $next)
    {
        ...
        /*
         * **تشديدُ #14 — رافعةٌ مطفأةٌ لا سياسةٌ مفروضة** (§٥ · v2.597.0).
         *
         * البندُ يقترح فرضَ 2FA على الرموز الصادرة سلفاً، وكلفتُه المعلَنة
         * «التكاملاتُ القائمةُ تتوقّف». **فالمفتاحُ مطفأٌ افتراضياً** — لا
         * يتغيّر شيءٌ بالترقية — و`HardeningReadiness::apiTokens()` يُخرج
         * قائمةَ مَن يتوقّف بالاسم قبل أن يُشعله أحد.
         *
         * ولا استثناءَ للمالك: من أشعل المفتاحَ يقرّر لنفسه كما لغيره،
         * والقائمةُ أمامه تُسمّيه إن كان رمزُه من المتوقّفين.
         */
        if ((string) setting('security.api_require_2fa', '0') === '1' && ! $user->totp_enabled) {
            return Api::error(Api::ACCOUNT_RESTRICTED, 403,
                'هذا المفتاح يتطلب تفعيل المصادقة الثنائية لصاحبه',
                ['reason' => 'token_owner_2fa_required']);
        }
