...
class ProfileController extends Controller
{
    ...
    {
        // (الجولة 1 · F23) حسابُ العميل لا يسكّ مفاتيحَ API: سطحُ `/api/v1` الداخليّ
        // ليس له، ومفتاحٌ بحسابه بابُ وصولٍ برمجيٍّ دائمٌ خارجَ عين البوّابة.
        // PortalGuard يستثني `profile.token.*` من قائمته البيضاء أصلاً — وهذا
        // دفاعٌ في العمق بنمطه: ٤٠٤ لا ٤٠٣ (لا نُثبت وجودَ السطح).
    ...
    {
        // (الجولة 1 · F23) التدويرُ سكٌّ بقيمةٍ جديدة — يُصدّ عن حساب العميل كالسكّ
        abort_if(hub_is_client(auth()->user()), 404);
