...
class QuoteBuilderController extends Controller
{
    ...
    public function storeLine(Request $r, string $id)
    {
        ...
        // (fieldsec/قواعدُ الدور): من يراها قراءةً فقط أو محجوبةً لا **يكتبها** من
        // باني البنود — كان `quotes:e` وحدَه يكفي فيلتفُّ على قاعدةِ الحقل. تعبئةُ
        // الكتالوجِ النظاميّةُ أدناه تبقى (قيمةُ النظامِ لا قيمةُ المُدخِل).
        if (hub_field_mode(auth()->user(), 'quotes', 'cost') !== '') {
            unset($d['unit_cost']);
        }
