...
class FileController extends Controller
{
    ...
    public function show(\Illuminate\Http\Request $r, string $path)
    {
        ...
        //
        // **FIO-1: الحاجزُ يغطّي المجموعةَ نفسَها التي يغطّيها إذنُ الوثيقة أدناه** — كان
        // يسأل النطاقَ الافتراضيّ (يعمى عن المحذوف ناعماً) وعمودَ `path` وحدَه، بينما
        // إذنُ `DocumentPolicy` على بُعدِ أسطرٍ يستعمل `withTrashed()` و`path OR thumb_path`.
        // فمرفقٌ مصابٌ حُذف ناعماً، أو **مصغّرةُ** مرفقٍ مصاب، كانا يتجاوزان الحجبَ ويُبَثّان.
        // القاعدة: الحاجزُ والإذنُ على المجموعة نفسِها (نظيرُ حارسِ الحذف الناعم · F-11).
        abort_if(\App\Models\Attachment::withTrashed()
            ->where(fn ($w) => $w->where('path', $path)->orWhere('thumb_path', $path))
            ->where('av_status', 'infected')->exists(),
        ...
        /*
         * **`withTrashed` ليست تساهلاً — هي شرطُ صحّةِ الحارس** (المراجعةُ الشاملة · F-11).
         *
         * قرارُ **الإذن** أدناه (`canRead`) يسأل `DB::table('attachments')` خامّاً فيرى
         * المحذوفَ ناعماً؛ وقرارُ **الحماية** هنا كان يسأل النموذجَ وفيه `SoftDeletes`
         * فيعماه. فحذفٌ ناعمٌ واحدٌ كان يُسقط `DocumentPolicy` ويُبقي الوصولَ: جوازُ
         * سفرٍ محجوبٌ بـ`docsec`، أو وثيقةٌ مُنع منها شخصٌ بعينه، تصير مقروءةً لكلِّ
         * من يملك صلاحيّةَ الوحدة.
         *
         * **القاعدة: الحارسُ يغطّي المجموعةَ نفسَها التي يغطّيها الإذن** — وإلّا فالفرقُ
         * بينهما بابٌ مفتوح. (وقد وُجد الحارسُ مكتوباً وتعليقُه أدناه يشرح الهجمةَ
