 ...
 *
 * **مستقلٌّ تماماً عن `/api/v1`:** لا يمسّ `App\Support\Platform\OpenApi` ولا
...
class MobileOpenApi
{
    ...
    private static function opMeta(): array
    {
        ...
        return [
            ...
            'mobile.ask.run' => ['tag' => 'ask', 'summary' => '«اسأل Hub»: سؤالٌ بلغتك يُجاب من بياناتك المُنطَّقة وحدَها — ومتابعةٌ في خيطٍ بـthread',
                'body' => $obj(['q' => $str, 'thread' => $strN], ['q']),
                'ok' => $env($obj(['ok' => $bool, 'answer' => $strN, 'partial' => $bool, 'failure' => $strN, 'message' => $str,
                    'sources' => ['type' => 'array', 'items' => $obj(['n' => $int, 'module' => $strN, 'label' => $strN, 'rows' => $int,
                        'ids' => ['type' => 'array', 'items' => $str], 'complete' => $bool])],
                    'thread' => $strN, 'request' => $str])), 'errors' => ['403', '404', '422']],
            'mobile.ask.threads.index' => ['tag' => 'ask', 'summary' => 'خيوطُ محادثاتي (لي وحدي · مشفَّرة · تُمحى بعد مدّة الاحتفاظ)',
                'ok' => $env($obj(['memory' => $bool, 'retention_days' => $int,
                    'threads' => ['type' => 'array', 'items' => $obj(['id' => $str, 'title' => $str, 'last_at' => $strN])]])), 'errors' => ['403']],
            'mobile.ask.threads.show' => ['tag' => 'ask', 'summary' => 'أدوارُ خيطي — والجوابُ المحفوظُ يُعاد تحقّقُه (hidden حين خرجت مصادرُه من نطاقي)',
                'ok' => $env($obj(['id' => $str, 'title' => $str, 'turns' => ['type' => 'array', 'items' => $obj([
                    'question' => $str, 'answer' => $strN, 'ok' => $bool, 'hidden' => $bool, 'failure' => $strN, 'at' => $strN])]])),
                'errors' => ['403', '404']],
            'mobile.ask.threads.destroy' => ['tag' => 'ask', 'summary' => 'محوُ خيطي نهائيّاً',
                'ok' => $env($obj(['deleted' => $int])), 'errors' => ['403', '404']],
            'mobile.ask.threads.destroy_all' => ['tag' => 'ask', 'summary' => 'محوُ كلِّ خيوطي نهائيّاً',
                'ok' => $env($obj(['deleted' => $int])), 'errors' => ['403']],
    ...
    private static function tags(): array
    {
        $t = [
            ...
            'home' => 'مساحةُ العمل',
            'search' => 'البحث',
            'ask' => '«اسأل Hub» — المساعدُ القارئ وذاكرةُ محادثاته',
     ...
     * الواحدُ `App\Support\Collaboration\NotificationLink::target`)، وترميزُها في رابطٍ عالميّ.
     */
    private static function deepLinkDoc(): array
    {
        return [
            'canonical' => ['module' => 'string (مفتاحُ وحدةٍ مسجَّلة)', 'id' => 'string (UUID)', 'action' => 'show'],
            'source_of_truth' => 'App\\Support\\Collaboration\\NotificationLink::target() — نفسُها التي تصدرها notifications/{id}/target والبحثُ واللوحة',
    ...
    public static function capabilities(): array
    {
        ...
        return [
            '_generator' => 'App\\Support\\Mobile\\MobileOpenApi::capabilities() — مُشتقٌّ من المسارات الحيّة (php artisan tinker / اختبار الانحراف)',
            ...
            'openapi' => ['live' => '/' . self::PREFIX . '/openapi.json', 'generator' => 'App\\Support\\Mobile\\MobileOpenApi::spec()'],
            'auth' => self::authFlowDoc(),
            'error_codes' => [
                'contract' => 'App\\Support\\Platform\\Api::CODES (موحَّدٌ مع /api/v1 — لا يُعاد تسميةُ كودٍ قائم)',
    ...
    private static function areaOf(string $name, string $uri): string
    {
        $map = [
            ...
            'mobile.ask.run' => 'ask', 'mobile.ask.threads.index' => 'ask', 'mobile.ask.threads.show' => 'ask',
            'mobile.ask.threads.destroy' => 'ask', 'mobile.ask.threads.destroy_all' => 'ask',
