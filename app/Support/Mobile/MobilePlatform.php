...
class MobilePlatform
{
    ...
    {
        return array_keys(\App\Support\Security\SecurityEvents::CODES);
    }
    ...
    public static function categoryLabel(?string $code): string
    {
        ...
        return \App\Support\Security\SecurityEvents::CODES[$code][0] ?? $code;
    }
     ...
     * `App\Support\Security\Identity` القائم). **لا يخزّن صوراً ولا يراقب** — حالةٌ لا مراقبة.
     */
    public static function scannerStatus(): array
    {
        ...
        return [
            ...
            'reuses'      => 'App\\Support\\Security\\Identity (حلُّ الهويّة الموحَّد — لا مخزنَ ثانٍ)',
        ];
    }
    ...
    private const AREA_TAB = [
        ...
        'work' => 'api',
        // «وثائقي» — خدمةٌ ذاتيّةٌ على سطحِ الأعمال (N-5 · تكافؤُ السطحَين v2.526.0)
        'me' => 'api',
