...
class CommentService
{
    ...
    /**
     * **حارسُ منشورِ القناةِ الموسومِ بشركة** (Permissions 360 · 08.2): `guardTarget`
     * يُعيد `['feed', null]` بلا تنطيقٍ — فالقناةُ عامّة — لكنّ صفَّ التعليقِ نفسَه قد
     * يحمل `company_id` (الطور A · SF-4: خلاصةُ كلِّ شركةٍ معزولة). القارئُ المحصورُ
     * بشركاتٍ لا يتفاعل مع منشورِ شركةٍ خارجَها (٤٠٤ كسائرِ العزل — لا يعلم بوجوده).
     * يُستدعى **بعد** `guardTarget` في مسارَي التفاعل (الويب والجوال) — العرضُ نفسُه
     * معزولٌ أصلاً في قارئِ الخلاصة؛ هذا يسدُّ التفاعلَ بالمعرّفِ المباشر.
     */
    public static function guardFeedComment(?User $actor, Comment $c): void
    {
        if ((string) $c->module !== 'feed' || ! $c->company_id) return;

        $cids = hub_company_ids($actor);
        abort_if($cids !== null && ! in_array((string) $c->company_id, $cids, true), 404);
    }
    ...
    private static function convMutes(?string $convId, string $uid, bool $isMention): bool
    {
        ...
        return $pref === \App\Support\Collaboration\Collaboration::NOTIFY_MUTED
            || ($pref === \App\Support\Collaboration\Collaboration::NOTIFY_MENTIONS && ! $isMention);
    }
