...
class Audit
{
        ...
        try {
            ...
            return ['ok' => false, 'state' => 'unknown', 'broken' => 0, 'label' => 'تعذّر الفحص',
                    'why' => 'تعذّر فحصُ سلسلة التدقيق ('
                        . \Illuminate\Support\Str::limit(\App\Support\Platform\Redactor::text($e->getMessage()), 120)
    ...
    protected static function show(string $col, $v): string
    {
        ...
        if (is_array($v)) return \Illuminate\Support\Str::limit(
            \App\Support\Platform\Redactor::text((string) json_encode($v, JSON_UNESCAPED_UNICODE)), 120);
        ...
        return \Illuminate\Support\Str::limit(\App\Support\Platform\Redactor::text($s), 140);
    }
