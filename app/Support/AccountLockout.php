...
class AccountLockout
{
    ...
    public static function bump(User $u): void
    {
        ...
        if ($n >= $max) {
            ...
            if ($locked) {
                /*
                 * **الكتابةُ الخامُّ تُبطِل خبيئتَها** (مجلس الخبراء · N-9): هذا
                 * المسارُ يكتب `users` بـ`DB::table` فلا يمرّ بخطّافِ النموذج الذي
                 * يستدعي `hub_data_bump`. فشاشةُ المستخدمين المحسوبةُ كانت تبقى
                 * تعرض الحسابَ «نشطاً» حتى تنتهي مهلةُ الخبيئة — والقفلُ واقعٌ.
                 */
                hub_data_bump('users');
