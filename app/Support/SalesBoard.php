...
class SalesBoard
{
    ...
    protected static function pipeline(): array
    {
        ...
        /*
         * **«٠» تعني «لم يُقَس» لا «صفرٌ متوقَّع»** (مجلس الخبراء · PROD-09).
         *
         * طبعت اللوحةُ **«التنبّؤ المرجَّح = 0.000»** بجانب خطِّ أنابيبَ خامٍ قدرُه
         * ٢٥١٬٠٠٠ — والسببُ أنّ **لا صفقةَ واحدةً تحمل احتمالَ إغلاق**. فالرقمُ
         * ليس تنبّؤاً بصفرٍ بل **غيابَ مُدخَل**، والفرقُ أنّ الأوّلَ قرارٌ والثاني
         * نقصُ بيانات. والمنتجُ يعرف الصوابَ ويستعمل «—» في مواضعَ أخرى.
         *
         * **ولا يُخفى شيء:** الرقمُ الخامُ باقٍ، والعدُّ باقٍ، والحقلُ باقٍ — وإنّما
         * يُعلَن أنّ المرجَّحَ لم يُقَس بعد، فيُرسَل البائعُ إلى **حقلِ الاحتمال**.
         */
        $hasProb = $rows->contains(fn ($r) => (float) ($r->prob ?? 0) > 0);

        return ['weighted' => $weighted, 'raw' => $raw, 'open' => $rows->count(),
                'weighted_measured' => $hasProb,
