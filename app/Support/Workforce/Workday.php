...
class Workday
{
    ...
    public static function checkOut(User $user): array
    {
        ...
        $c = \App\Support\Workforce\DailyWorkCompliance::resolve($emp, (string) ($row->date?->toDateString() ?? $row->date));
        $pendingNote = '';
        if (in_array($c['state'], [\App\Support\Workforce\DailyWorkCompliance::REPORT_PENDING,
            \App\Support\Workforce\DailyWorkCompliance::PRESENT_WITHOUT_REPORT], true)) {
    ...
    public static function onLeave(string $empId, string $date): bool
    {
        ...
        return LeaveRequest::whereNull('deleted_at')->where('emp_id', $empId)
            ...
            ->tap(fn ($q) => \App\Support\Platform\DayRange::upto($q, 'date_from', $date))
            ->where(fn ($q) => \App\Support\Platform\DayRange::since($q, 'date_to', $date)->orWhereNull('date_to'))
            ->exists();
    }
        ...
        Employee::whereNull('deleted_at')->where('status', 'نشط')
            ->orderBy('id')->chunkById(100, function ($emps) use ($date, $crossers, &$n) {
                foreach ($emps as $emp) {
                    ...
                    if (\App\Support\Workforce\DailyWorkCompliance::excuseFor($emp->id, $date) !== null) continue;
                    Attendance::create([
    ...
    protected static function teamCalc(): array
    {
        ...
        $comp = \App\Support\Workforce\DailyWorkCompliance::resolveMany($emps, $today);
        ...
        $blockersByUser = DB::table('work_updates')->whereNull('deleted_at')
            ->tap(fn ($q) => \App\Support\Platform\DayRange::on($q, 'work_date', $today))
         ...
         */
        $roll = \App\Support\Workforce\DailyWorkCompliance::rollCall($emps, $today);
        ...
        foreach ($emps as $e) {
            ...
            if ($c['state'] === \App\Support\Workforce\DailyWorkCompliance::PRESENT_WITHOUT_REPORT) $n['missing']++;
