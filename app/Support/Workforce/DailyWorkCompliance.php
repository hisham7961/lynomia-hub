...
class DailyWorkCompliance
{
    ...
    public static function excusedMap(array $empIds, string $from, ?string $to = null): array
    {
        ...
        $rows = \App\Models\LeaveRequest::whereNull('deleted_at')->whereIn('emp_id', $empIds)
            ...
            ->tap(fn ($q) => \App\Support\Platform\DayRange::upto($q, 'date_from', $to))
            ->where(fn ($q) => \App\Support\Platform\DayRange::since($q, 'date_to', $from)->orWhereNull('date_to'))
    ...
    public static function onLeaveToday(?string $date = null, $companyId = null): Collection
    {
        ...
        return \App\Models\LeaveRequest::query()->whereNull('leave_requests.deleted_at')
            ...
            // `leave_requests_emp_dates_idx` أُضيف في هذه الدفعة، وقِيس بـ`EXPLAIN`
            // أنّ المدى يُنصّف الصفوفَ المفحوصةَ فوق ما يكسبه الفهرسُ وحدَه.
            ->tap(fn ($q) => \App\Support\Platform\DayRange::upto($q, 'leave_requests.date_from', $date))
            ->where(fn ($q) => \App\Support\Platform\DayRange::since($q, 'leave_requests.date_to', $date)
