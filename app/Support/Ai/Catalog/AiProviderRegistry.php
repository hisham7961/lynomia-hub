...
final class AiProviderRegistry
{
    ...
    /** موضعُ الشعاراتِ داخلَ `public/` */
    public const LOGO_DIR = 'img/ai/providers';

    /** ذاكرةُ الطلبِ الواحد — الشاشةُ تعرض خمسةً وتسعين شعاراً في تمريرةٍ واحدة */
    private static array $svgMemo = [];

    /**
     * **اسمُ ملفِّ شعارِ هذا المزوّد** — أو `null` إن لا شعارَ له.
     *
     * والخريطةُ مولَّدةٌ (`hub:ai-logos`) من مفاتيحِ القياسِ وأسماءٍ مكتوبةٍ
     * بيدٍ ومُراجَعة. **ومفتاحٌ لا سطرَ له يعود `null`** فيسقط إلى العلامةِ
     * الحرفيّة — ولا يُستعار له شعارُ جارِه.
     */
    public static function logo(string $key): ?string
    {
        $file = config('ai_logos.logos.' . self::hubKey($key));

        return is_string($file) && $file !== '' ? $file : null;
    }

    /**
     * **متنُ الشعارِ جاهزاً للإدراجِ في الصفحة** — أو `null`.
     *
     * ── **ولمَ يُدرَج المتنُ بدل `<img src>`؟** ──
     *
     * لأنّ أحدَ عشرَ شعاراً في المجموعةِ **أحاديُّ اللونِ بـ`currentColor`**:
     * علاماتٌ صمّمها أصحابُها بلا لون. وفي `<img>` تكون الصورةُ **مستنداً
     * منفصلاً** لا يرث لونَ صفحتِنا، فيُرسَم أسودَ على خلفيّةٍ داكنةٍ —
     * أي **يختفي**. والإدراجُ يجعلها ترث لونَ البطاقة، فتظهر في الوضعين.
     *
     * **والإدراجُ آمنٌ هنا بثلاثةِ شروطٍ مجتمعة**: الملفُّ **مُثبَّتٌ في
     * المستودعِ** لا مجلوبٌ من شبكة، واسمُه يأتي من **خريطةٍ مولَّدةٍ** لا من
     * مُدخَل (فلا اجتيازَ مسار)، ومتنُه يمرّ بحارسٍ يرفض ما ليس `<svg` نظيفاً.
     * وحارسٌ رابعٌ في الحزمةِ يمسح الملفّاتِ كلَّها عند كلِّ تشغيل.
     */
    public static function logoSvg(string $key): ?string
    {
        $file = self::logo($key);
        if ($file === null) return null;

        if (array_key_exists($file, self::$svgMemo)) return self::$svgMemo[$file];

        // **اسمُ ملفٍّ لا مسار** — والخريطةُ مولَّدةٌ، لكنّ الحارسَ لا يفترض
        if ($file !== basename($file)) return self::$svgMemo[$file] = null;

        $path = public_path(self::LOGO_DIR . '/' . $file);
        $svg  = is_file($path) ? (string) file_get_contents($path) : '';

        return self::$svgMemo[$file] = self::safeSvg($svg) ? $svg : null;
    }

    /**
     * **يُفرِغ ذاكرةَ المتون.**
     *
     * الذاكرةُ ساكنةٌ فتعبر بين أصنافِ الاختبارِ في العمليّةِ الواحدة — وهو
     * الصوابُ في الإنتاج (طلبٌ واحدٌ لا يقرأ الملفَّ مرّتين) **والخطأُ في
     * القياس**: صنفٌ يُبدّل الخريطةَ ليُثبِت أنّ الحارسَ يردّ، فيُجاب من
     * قراءةٍ سابقةٍ نجحت. ويُستدعى هذا في الاختبارِ وحدَه.
     */
    public static function flushLogoMemo(): void
    {
        self::$svgMemo = [];
    }

    /**
     * **أهذا المتنُ شعارٌ نظيف؟** — شرطٌ يُفحَص لا يُفترَض.
     *
     * ولا يُقصَد به تطهيرُ مدخلٍ معادٍ: الملفّاتُ مُراجَعةٌ ومُختبَرة. يُقصَد به
     * أن **يبقى الشرطُ مكتوباً** فلا يمرّ ملفٌّ بُدِّل يوماً بلا أن يُلاحَظ.
     */
    public static function safeSvg(string $svg): bool
    {
        $s = mb_strtolower($svg);

        if (! str_starts_with(ltrim($s), '<svg')) return false;

        foreach (['<script', '<foreignobject', 'javascript:', '<iframe', '<use '] as $bad) {
            if (str_contains($s, $bad)) return false;
        }

        // معالجاتُ أحداثٍ من أيِّ نوع
        return preg_match('/\son[a-z]+\s*=/', $s) !== 1;
    }

    /**
     * **علامةُ المزوّدِ البصريّة** — حرفٌ أو حرفان ولونٌ، مشتقّانِ من اسمِه.
     *
     * ── **وهي اليومَ احتياطٌ لا أصل** ──
     *
     * كانت هذه العلامةُ هي ما يُعرَض للمزوّدين جميعاً، والحجّةُ أنّ شعاراتِ
     * مئةٍ وستّةٍ وعشرين علامةً تجاريّةً أصولٌ ثقيلةٌ تتقادم. وقرّر المالكُ
     * الشعاراتِ الحقيقيّة، فصارت تُثبَّت في المستودعِ بخريطةٍ مولَّدةٍ
     * (`hub:ai-logos`) وإسنادٍ معلَن.
     *
     * **والعلامةُ الحرفيّةُ بقيت — ولم تُحذَف — لأنّ الحجّةَ الأصليّةَ صحّت
     * جزئيّاً**: ستّون مزوّداً لا شعارَ لهم في أيِّ مجموعةٍ مفتوحة، ومزوّدٌ
     * يظهر غداً لن يكون له ملفٌّ أصلاً. فلولا هذا الاحتياطُ **لبقي ثُلثُ
     * الشاشةِ مربّعاتٍ فارغة** — وهو ما يمنعه التصميمُ الأصليُّ بالضبط.
     *
    ...
    public static function renderMap(array $measured): string
    {
        ...
        $out .= "        'classifier'      => 'App\\\\Support\\\\Ai\\\\Catalog\\\\AiProviderRegistry::classify',\n";
