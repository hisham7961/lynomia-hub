...
class StepUp
{
    ...
    /**
     * **فعلُ التدقيقِ لفشلِ التصعيد — صياغةٌ واحدةٌ يملكها هذا الصنف.**
     *
     * كان مكتوباً حرفيّاً في موضعَين: «فشل تصعيد المصادقة» في مسارِ الويب،
     * و«فشلُ تصعيد المصادقة» (بحركةٍ زائدة) في مسارِ الجوال. و`SecurityEvents`
     * يطابق أفعالَ التدقيق بـ`whereIn('audits.action', …)` — **مطابقةٌ نصّيّةٌ
     * تامّة** — وكتالوجُه يحمل الصياغةَ الأولى وحدَها.
     *
     * **فكان فشلُ تصعيدِ المصادقةِ من الجوال لا يُسجَّل حدثاً أمنيّاً
     * إطلاقاً**: لا يظهر في الرادار، ولا تلتقطه قاعدةُ تنبيه، وشدّتُه
     * `warning`. حرفٌ واحدٌ صنع نقطةً عمياء.
     *
     * والصياغةُ المُبقاةُ هي صياغةُ الويب: هي ما في الكتالوج، وما في الاختبارِ
     * القائم، **وما في صفوفِ الإنتاج المكتوبةِ أصلاً** — فتغييرُها يقطع
     * التاريخ، وتغييرُ الأخرى إضافةٌ لا كسر.
     */
    public const AUDIT_FAILURE = 'فشل تصعيد المصادقة';
    ...
    public static function checkCredential($user, string $input): bool
    {
        ...
        return $user->totp_enabled
            ? \App\Support\Security\Totp::verifyOnce((string) $user->totp_secret_cipher, $input, 'stepup:' . $user->id)
