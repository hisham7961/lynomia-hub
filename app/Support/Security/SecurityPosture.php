...
class SecurityPosture
{
    ...
    protected static function idleUsers(): array
    {
        ...
        // **والوسمُ يقول العتبةَ التي عُدَّ بها** (#٣٥أ): كان نصّاً ثابتاً «٦٠ يوماً»
        // بينما العدُّ يجري بالمضبوطة — فمن خفضها قرأ رقمَ ثلاثين تحت عنوانِ ستّين.
        return self::row('idle', "حسابات نشطة بلا دخولٍ {$idleDays} يوماً", $n ? ($n > 2 ? 'bad' : 'wn') : 'ok',
    ...
    protected static function backupFresh(): array
    {
        ...
        /*
         * **النبضةُ تقول «نجحتُ» ولا تقول «ثمّة ما يُستعاد»** (مجلس الخبراء · ت-٣).
         *
         * كان هذا الفحصُ يسأل الختمَ وحدَه، فقال «✅ حداثةُ النسخة» بينما
         * `storage/app/backups` **فارغٌ تماماً** — وأيُّ سببٍ لفقدِ الأثر (تدويرٌ
         * معطوب · تنظيفُ قرص · تغيّرُ مسارٍ بعد نشر · حذفٌ يدويّ) كان يترك الشاشةَ
         * مطمئنّةً على ما لا رجعةَ فيه. **والأثرُ يُقرأ الآن من السلطةِ الواحدةِ**
         * (`hub_backup_artifacts`) التي يقرؤها مركزُ التشغيلِ نفسُه — فلا تعريفان.
         *
         * **ولا قدرةَ تُنزع:** الصفُّ باقٍ وعدّادُ مركزِ التشغيلِ باقٍ — يضيق
         * تعريفُ «سليم» ليصدق فقط.
         */
        $art = hub_backup_artifacts();
        // **الكذبُ المستهدَفُ بدقّة:** نبضةٌ تقول «أُخذت» وقرصٌ لا يحمل شيئاً.
        // أمّا «لم تُؤخذ قطّ» (`$ageMin === null`) فالشاشةُ تحذّر منها أصلاً
        // **وهي صادقة** — فلا تُصعَّد، ولا يُغيَّر سلوكُ تنصيبٍ جديدٍ لم يبدأ بعد.
        if ($ageMin !== null && $art['count'] < 1) {
            $tone = 'bad';
            $fix = 'النبضةُ تقول إنّ النسخةَ أُخذت، و**لا ملفَّ نسخةٍ واحداً** في '
                 . 'storage/app/backups — فلا شيءَ يُستعاد. افحص التدوير وصلاحيّاتِ '
                 . 'المجلّد ومساحةَ القرص، ثمّ اضغط «نسخة الآن» في مركز التشغيل وتأكّد '
                 . 'من ظهور الملفّ.';
        }

        // القيمةُ تحمل شاهدَها: «٣ ملفّات» أو «٠ ملفّ» — فالعلامةُ العاريةُ لا تُكذَّب
        $value = $art['count'] . ' ملفّ' . ($art['count'] === 1 ? '' : 'اً');
        ...
        return self::row('backup_fresh', 'حداثةُ النسخة الاحتياطية', $tone,
            ...
            $tone === 'ok' ? 0 : 1, $fix, route('ops.index'), $value);
    }
