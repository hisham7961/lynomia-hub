...
if (! function_exists('hub_top_links')) {
    ...
    function hub_top_links($user): array
    {
        ...
        $all = [
            ...
            // **اسأل Hub** (المرحلة ٣ · P3-W6) — وجهةٌ يوميّةٌ لا إعدادٌ إداريّ:
            // يسأل الموظّفُ عن بياناتِه بصلاحيّتِه هو. **وشرطُ العرضِ شرطُ الباب
            // حرفاً بحرف**: `canAsk` وحدَها — أمّا التوافرُ (بوّابةٌ جاهزةٌ وغرضٌ
            // مُهيَّأ) فيُقال **داخلَ الصفحة**، لأنّ إخفاءَ الرابطِ عند انقطاعِ
            // خدمةٍ يجعل الميزةَ تختفي فيظنُّ صاحبُها أنّه فقد صلاحيّتَها.
            ['key' => 'ask',       'label' => '🤖 اسأل Hub',          'route' => 'ask.index',       'group' => 'daily',
                'ok' => \App\Support\Ai\Ask\AskPolicy::canAsk($user)],
...
if (! function_exists('hub_expiry_self_scan')) {
    /** غلافٌ — المنطقُ في `ExpiryRadar::selfScan` (docs/REORG_PLAN.md §R5) */
    function hub_expiry_self_scan(array $md, $user): array
    {
        return \App\Support\Insights\ExpiryRadar::selfScan($md, $user);
    }
}
...
if (! function_exists('hub_expiry')) {
    /** غلافٌ — المنطقُ في `ExpiryRadar::scan` (docs/REORG_PLAN.md §R5) */
    function hub_expiry(bool $fresh = false, $user = null): array
    {
        return \App\Support\Insights\ExpiryRadar::scan($fresh, $user);
    }
}
...
if (! function_exists('hub_health')) {
    /** غلافٌ — المنطقُ في `CompanyHealth::compute` (docs/REORG_PLAN.md §R5) */
    function hub_health(bool $fresh = false): array
    {
        return \App\Support\Insights\CompanyHealth::compute($fresh);
    }
}
...
if (! function_exists('hub_mrr')) {
    /** غلافٌ — المنطقُ في `ProjectEconomics::mrr` (docs/REORG_PLAN.md §R5) */
    function hub_mrr(bool $fresh = false): array
    {
        return \App\Support\Insights\ProjectEconomics::mrr($fresh);
    }
}
...
if (! function_exists('hub_project_pl')) {
    /** غلافٌ — المنطقُ في `ProjectEconomics::pl` (docs/REORG_PLAN.md §R5) */
    function hub_project_pl(string $projectId, bool $fresh = false): array
    {
        return \App\Support\Insights\ProjectEconomics::pl($projectId, $fresh);
    }
}
...
if (! function_exists('hub_project_health')) {
    /** غلافٌ — المنطقُ في `ProjectEconomics::health` (docs/REORG_PLAN.md §R5) */
    function hub_project_health(string $projectId, bool $fresh = false): array
    {
        return \App\Support\Insights\ProjectEconomics::health($projectId, $fresh);
    }
}
...
if (! function_exists('hub_ar_norm_map')) {
    /**
     * جدولُ التطبيعِ العربيِّ الذي يشترك فيه PHP وSQL — **مصدرٌ واحد**، فلا يفترق
     * ما يُطبِّعه المحرّكُ عمّا يُطبِّعه `hub_ar_norm` فيصير البحثُ قرعةً أخرى.
     * المفتاحُ يُحذَف إن كانت قيمتُه `''` وإلا يُستبدل.
     */
    function hub_ar_norm_map(): array
    {
        return [
            // التشكيلُ والتطويل — تُحذَف
            "\u{064B}" => '', "\u{064C}" => '', "\u{064D}" => '', "\u{064E}" => '',
            "\u{064F}" => '', "\u{0650}" => '', "\u{0651}" => '', "\u{0652}" => '',
            "\u{0670}" => '', "\u{0640}" => '',
            // الهمزاتُ والألفُ المقصورةُ والتاءُ المربوطة — تُوحَّد
            'أ' => 'ا', 'إ' => 'ا', 'آ' => 'ا', 'ٱ' => 'ا', 'ى' => 'ي', 'ة' => 'ه',
        ];
    }
}

if (! function_exists('hub_ar_norm_sql')) {
    /**
     * **تطبيعُ العربيّةِ داخلَ الاستعلام** — `REPLACE` متداخلةٌ تعمل حرفيّاً على
     * المحرّكين (MySQL وSQLite) بلا دالّةٍ خاصّةٍ بأحدهما ولا عمودٍ مولَّد.
     *
     * ولمَ لزم؟ لأنّ `LIKE` تقارن الحرفَ بالحرف: مشروعُ **«بوّابة الخليج»** مخزَّنٌ
     * بالشدّة، ومن يكتب «بوابة الخليج» بلا شدّةٍ — وهو ما يكتبه كلُّ أحد — لا يجد
     * شيئاً. لا رسالةَ ولا تلميح، بل «لا نتائج» على سجلٍّ قائمٍ أمامه. وكذلك
     * «احمد» لا تجد «أحمد». فالمنتجُ يملك `hub_ar_norm` منذ البداية ويستعملها في
     * مطابقةِ الحالاتِ والتدفّقات — **وكان البحثُ هو البابَ المنسيّ**.
     *
     * والكلفةُ مقبولةٌ لأنّها لا تُغيّر رتبةَ التعقيد: `LIKE '%…%'` أصلاً مسحٌ
     * كامل — البادئةُ الحرّةُ تُبطل الفهرس — فالتطبيعُ يضيف عملاً على الصفِّ لا
     * صفوفاً على المسح. ولا يُستدعى إلا حين يحوي المصطلحُ حرفاً عربيّاً.
     */
    function hub_ar_norm_sql(string $col): string
    {
        $expr = $col;
        foreach (hub_ar_norm_map() as $from => $to) {
            // القيمتان حرفيّتان من جدولٍ ثابتٍ في الشيفرة — لا مدخلَ مستخدمٍ هنا
...
if (! function_exists('hub_has_arabic')) {
    /** هل في النصِّ حرفٌ عربيّ؟ — فلا يدفع البحثُ اللاتينيُّ كلفةَ التطبيع */
    function hub_has_arabic(?string $s): bool
    {
...
if (! function_exists('hub_timeline')) {
    /** غلافٌ — المنطقُ في `RecordTimeline::build` (docs/REORG_PLAN.md §R5) */
    function hub_timeline(string $module, string $recordId, int $limit = 60): array
    {
        return \App\Support\Platform\RecordTimeline::build($module, $recordId, $limit);
    }
}
...
if (! function_exists('hub_capacity')) {
    /** غلافٌ — المنطقُ في `ProjectEconomics::capacity` (docs/REORG_PLAN.md §R5) */
    function hub_capacity(?string $from = null, ?string $to = null, ?string $projectId = null): array
    {
        return \App\Support\Insights\ProjectEconomics::capacity($from, $to, $projectId);
    }
}
...
if (! function_exists('hub_service_costs')) {
    /** غلافٌ — المنطقُ في `ProjectEconomics::serviceCosts` (docs/REORG_PLAN.md §R5) */
    function hub_service_costs(bool $fresh = false): array
    {
        return \App\Support\Insights\ProjectEconomics::serviceCosts($fresh);
    }
}
...
if (! function_exists('hub_recommendations')) {
    /** غلافٌ — المنطقُ في `Recommendations::build` (docs/REORG_PLAN.md §R5) */
    function hub_recommendations(bool $fresh = false, ?string $projectId = null): array
    {
        return \App\Support\Insights\Recommendations::build($fresh, $projectId);
    }
}
...
if (! function_exists('hub_admin_links')) {
    ...
    function hub_admin_links($user): array
    {
        ...
        return [
            ...
            /*
             * أسعارُ الصرف (v2.528.0): بابٌ يُدخِل السعرَ — ولولا هذا السطرِ لكان
             * المحرّكُ ميزةً مخفيّة، وهو الصنفُ الذي لاحقته الجلسةُ كلُّها.
             *
             * وشرطُه `$owner` لا `fin:v` — تصحيحٌ قبل الدفع: **الوقوفُ في شريطِ
             * الإدارة يُشتقُّ من مِلكيّةٍ أو راية، لا من مصفوفةِ وحدة.** فـ`fin:v`
             * صادقٌ لأيِّ موظّفٍ دورُه يرى الماليةَ، وبه صار `hub_admin_bar_visible`
             * صادقاً له فانفتح **مجالُ الإدارةِ كلُّه** لموظّفٍ عاديّ — كشفته أربعُ
             * بوّابات: NavCatalog وIaGuardParity وIaAdminGate وMobileIa.
             *
             * ومقامُه «الإعدادات» لا مجموعةٌ خامسة: السعرُ إعدادٌ يحكم تحويلَ كلِّ
             * مبلغ، فمكانُه بين أخواتِه المحروسةِ بالمالك، وكبسولاتُ §11 أربعٌ.
             */
            $mk('curfx', 'أسعار الصرف', '💱', 'currency.rates', [], 'الإعدادات',
                $owner, ['currency.rates*'], 'عملات صرف تحويل'),
            ...
            // مركزُ الذكاء الاصطناعيّ (v2.559 · وW8): حارسُه حارسُ صفحتِه حرفاً
            // بحرف — فلا رابطٌ يظهر ثمّ يُصَدُّ ٤٠٣. **والوجهةُ صارت «نظرة»**
            // وبابُها القراءةُ، فيُضاف حاملُ `aiView` إلى شرطِ الرابطِ كما أُضيف
            // إلى شرطِ الباب. والشرطُ يُقرَأ من `AiAccess` لا يُنسَخ هنا.
            $mk('ai', 'الذكاء الاصطناعي', '🤖', 'ai.index', [], 'الإعدادات',
                \App\Support\Ai\Center\AiAccess::canView($user), ['ai.*'],
                'الذكاء الاصطناعي النماذج البوابة LiteLLM المزودون مساعد توجيه أغراض استهلاك تشخيص'),
