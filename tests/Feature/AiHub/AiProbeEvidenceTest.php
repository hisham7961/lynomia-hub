...
class AiProbeEvidenceTest extends TestCase
{
    ...
    public function test_كلُّ_لقطةِ_إكمالٍ_في_الحزمةِ_تحمل_ما_يضمنه_العقد(): void
    {
        ...
        foreach (array_unique($files) as $f) {
            ...
            /*
             * **الملفُّ الذي يُنشئ لقطةً وحدَه يُحاسَب** — لا من يفهرس فيها.
             *
             * `'choices' =>` إعلانُ لقطةٍ، و`['choices'][0]` تعديلُ لقطةٍ
             * أنشأها غيرُه (مُنشئٌ مشتركٌ مُحاسَبٌ في ملفِّه). وخلطُهما يُنذر
             * كاذباً على ملفٍّ سليم — **والإنذارُ الكاذبُ يُدرّب على التجاهل**.
             */
            if (! preg_match("~'choices'\\s*=>~", $src)) continue;
