...
class DogfoodFinaleExcuseTest extends TestCase
{
    ...
    /* ═══════════ 4ب) الصفحةُ الواحدةُ لا تقول عن الشخصِ الواحدِ قولين ═══════════ */

    /**
     * **نصفُ إصلاحٍ أسوأُ من لا إصلاح** (الخاتمة · X1ب — كشفته مراجعةُ الإدارة).
     *
     * أدخلتُ فئةَ «مأذون» في `rollCall` ونسيتُ `deriveEffective`. فصارت صفحةُ
     * `/workforce` تعرض هدى في «🪪 مأذون» في نداءِ اليوم، وتعرضها في الجدولِ
     * **تحتَه مباشرةً** بـ«الحالة المحتسَبة: **غائب**». صفحةٌ واحدةٌ · شخصٌ واحدٌ ·
     * يومٌ واحدٌ · حكمان متضادّان — وهو **عينُ عيبِ «المصدرَين»** الذي جاء الإصلاحُ
     * ليُنهيه، أعدتُ إنتاجَه داخلَ الشاشةِ الواحدة.
     *
     * **والأثرُ ليس تجميليّاً:** عمودُ «الحالة المحتسَبة» هو ما **يُصدَّر** وما
     * تُبنى عليه المحاسبةُ الشهريّة — فالمأذونُ كان يُصدَّر غائباً إلى الرواتب،
     * وهو بالضبط الضررُ الذي زعمتُ أنّي أغلقتُه.
     *
     * **ومفردةٌ واحدةٌ لا مفردتان:** خريطةُ التسمياتِ كانت تحمل `'excused' =>
     * 'معذور'` (فرعاً ميّتاً لا يصله شيء) بينما النداءُ يقول «مأذون». ولفظان
     * لحالةٍ واحدةٍ عيبٌ عرفه هذا المستودعُ من قبل (v2.165: «معلّق/مُقَر» مقابل
     * «بانتظار الإقرار/مُقرّة») — فوُحِّدت على **«مأذون»**.
     */
    public function test_the_computed_state_column_agrees_with_the_roll_call_about_one_person(): void
    {
        $this->seedCore();
        $this->hubSetting('sec.hours_start', '08:00');
        Carbon::setTestNow(Carbon::parse('2026-09-14 09:30:00', config('app.timezone')));
        ...
        $c = DailyWorkCompliance::resolve($huda);
        $roll = $this->roll();

        $this->assertContains($huda->name, $this->names($roll, 'excused'), 'النداءُ يقول «مأذون»');
        $this->assertSame('excused', $c['effective'],
            'والجدولُ تحتَه يقول «غائب» — صفحةٌ واحدةٌ بحكمين. وهذا العمودُ هو ما يُصدَّر للرواتب.');
        $this->assertSame('مأذون', $c['labels']['effective'],
            'ومفردةٌ واحدةٌ للحالةِ الواحدة: النداءُ «مأذون» فلا يقول الجدولُ «معذور»');

        // والغائبُ حقّاً يبقى غائباً في العمودِ نفسِه
        [, $plain] = $this->linkedEmployee('غائبٌ بلا طلبٍ البتّة');
        $this->assertSame('absent', DailyWorkCompliance::resolve($plain)['effective']);
    }
